        ...
        >
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-sm font-medium transition-colors hover:text-emerald-400 text-zinc-400 hover:text-white"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="hidden md:flex items-center gap-4">
            {user ? (
              <Link
                href="/dashboard"
                className="px-5 py-2 rounded-full bg-emerald-500 hover:bg-emerald-400 text-black text-sm font-bold transition-all shadow-[0_0_20px_-5px_rgba(16,185,129,0.4)]"
              >
                Dashboard
              </Link>
            ) : (
        ...
        {mobileMenuOpen && (
          ...
          >
            <motion.div
              ...
              className="absolute top-24 left-4 right-4 bg-[#111] border border-zinc-800 rounded-3xl p-6 shadow-2xl"
              ...
              <div className="flex flex-col gap-2">
                {navLinks.map((link) => (
                  <a
                    ...
                    href={link.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className="p-4 rounded-xl hover:bg-white/5 text-lg font-medium text-zinc-300 hover:text-white transition-all"
                  >
                    {link.name}
                  </a>
                ))}
                ...
                {user ? (
                  <Link href="/dashboard" className="p-4 text-center rounded-xl bg-emerald-500 text-black font-bold">
                    Dashboard
                  </Link>
                ) : (
                  <div className="flex flex-col gap-3">
                    ...
                    <Link href="/signup" className="p-3 text-center rounded-xl bg-emerald-500 text-black font-bold shadow-[0_0_20px_rgba(16,185,129,0.3)]">
                      Start Free
                    </Link>
                  </div>
                )}
